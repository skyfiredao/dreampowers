...
5. **检查前序章节摘要符号链接**：检查当前章节文件夹内是否已存在 `summary-*.md` 符号链接（正常情况下由上一章的 `dp-chapter-summary` 在第六步创建）。如果缺失：检查 `docs/dreampowers/timeline/` 中已存在的 `summary-*.md` 文件，为当前章节文件夹补建前 1-3 章摘要的符号链接（`summary-NNN.md → ../../timeline/summary-NNN.md`），不足 3 章按实际数量链接。写第一章时无前序章节，跳过本步骤
6. **读取前序章节摘要**：从章节文件夹内的 `summary-*.md` 符号链接读取摘要全文，确认前章结尾状态、角色位置、悬而未决的线索
...
17. **写入 spec.md 第七节（本章写作蓝图）**：将以上步骤的结果按以下子项组织写入：
    ...
    - **前情衔接**：前序摘要全文引用（每章≤150字，最多3章，总计≤450字）+ 需要衔接的具体事项
...
6. **续写衔接**：建立交接点，总结末章结尾状态，下一章进入正常 草稿预审
