...
- **伏笔**: [植入的伏笔] → [预计回收章节]
- **章末钩子**: [让读者翻到下一章的理由]
- **角色章末状态**: [本章结束时各主要角色的物理状态、情绪状态、所在位置。下一章开头必须延续这些状态]（此为大纲规划预设；章节写完后，dp-chapter-summary 会在摘要中生成 `[章末状态]` 实际快照，下游技能以摘要中的实际快照为准）
...
- 角色弧线的方向是否与核心冲突吻合？
- 有没有角色在某一卷完全消失？
- 相邻章节的角色章末状态是否连贯？（上章角色受伤，下章开头必须延续伤势；上章角色在 A 地，下章若在 B 地必须有合理位移）
...
2. 章末钩子是否每章都有？有没有章节遗漏？角色章末状态是否每章都有标注？
...
大纲和伏笔源文件更新后，必须同步更新所有**未写章节**的文件夹（`review_status` 不为 `released` 或 `imported` 的章节）：

1. **伏笔符号链接**：
   - 删除指向 `abandoned` 伏笔的符号链接
   - 为修订后新增的伏笔在相关章节文件夹中添加符号链接
   - 若伏笔的预期回收章节范围（tier）变更，调整链接分配（移除不再相关的章节链接，添加新相关的章节链接）

2. **概念/角色符号链接**：
   - 若修订导致某章的概念白名单或角色白名单变更（如角色提前出场、概念推迟引入），更新对应章节文件夹中的符号链接

3. **spec.md 第一～五节**：
   - 重写受影响章节的 spec.md 框架（概念预算、门控标准、概念依赖等），使其与修订后的大纲一致

4. **验证**：
   - 所有符号链接指向实际存在的源文件（无断链）
   - 无指向 `abandoned` 伏笔的残留链接

已写章节的工作区不在此步修改——它们在第六步"回写"时通过完整的 `dp-chapter-draft` 流程重建。
