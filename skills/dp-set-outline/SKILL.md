...
2. **大纲结构构建** — 结构模型、幕/卷划分、章节细化、POV 策略
3. **主题编织** — 核心主题与反题、主题追踪
4. **章节工作区创建** — 章节文件夹、符号链接、spec.md
...
<HARD-GATE>
在确认上述五项前置条件全部满足且已读取之前，禁止开始大纲构建。不论用户多急，前置条件不可绕过。
</HARD-GATE>
...
- [ ] 发掘核心主题与反题，定义主题层次
- [ ] 制定世界观揭示时间表
- [ ] 确定故事结构模型（三幕/四幕/英雄之旅/起承转合/自定义）
- [ ] 确定 POV 策略
- [ ] 划分幕/卷，每幕确定：核心冲突升级、角色弧线进展、世界观揭示节点、主题测试点
- [ ] 细化到章节级：每章的场景列表、POV 角色、情绪走向、时间线手法
...
### 铁律一：先好奇，后解释（CURIOSITY_BEFORE_EXPLANATION）
- 每个世界观元素必须先以谜团/悬念形式出现。
- 至少经过 1-2 个场景后才解释其原理。
- 读者应该先问"这是什么？"再得到答案。

### 铁律二：先感知，后体系（SENSE_BEFORE_SYSTEM）
- 角色先通过感官体验世界元素（看到、听到、感受到）。
- 再逐步理解背后的体系/规则。
- 禁止用旁白直接解释体系。

### 铁律三：扩展已有，少添新增（EXPAND_DONT_ADD）
- 优先深化已引入的概念，而非引入新概念。
- 旧概念的新层面 > 全新概念。
- 每次"深化"都应使已知概念变得更复杂、更有趣、更有代价。

### 铁律四：概念预算（CONCEPT_BUDGET）
- 第一章：最多引入 3 个新概念（可以为 0）。
- 后续每章：最多引入 2 个新概念（可以为 0）。
- 每个场景：最多解释 1 个已引入的概念。
- 每 3000 字：至少保留 1 个未解答的问题。

### 铁律五：禁止旁白讲解（NO_NARRATOR_LECTURES）
世界观信息只能通过以下方式传递：
- a. 角色对话（自然提及，非教学式）
- b. 角色行动（通过做事展示规则）
- c. 角色观察（通过感知发现）
...
3. **主题必须被测试** — 测试时机：第二幕中期（主角遭遇反题论证）、至暗时刻（主角几乎被反题说服）、高潮（主角在理解反题后重新选择或放弃核心主题）。
...
第一步是和用户确定故事的结构骨架。不要替用户选。给出选项，让用户挑。

| 模型 | 适用场景 | 结构 |
|------|---------|------|
| 三幕式 | 电影感强的中短篇 | 建立(25%) → 对抗(50%) → 解决(25%) |
| 四幕式 | 网文长篇 | 起(15%) → 承(35%) → 转(35%) → 合(15%) |
| 英雄之旅 | 成长型主角 | 12 阶段循环 |
| 起承转合 | 中式传统叙事 | 4 段式 |
| 自定义 | 实验性/非线性 | 用户自定义 |
...
> **注意**：故事级时间线（年代、时间跨度、关键日期）已在 `dp-set-concept` 阶段构建并存储于 `timeline/timeline.md`。本节只处理**章节级叙事手法**——每章采用顺叙、倒叙、插叙还是补叙。两者的区分：故事级时间线是"什么时候发生了什么"，章节级时间线手法是"怎么讲这些事"。
...
1. **同一章内不切换 POV。** 除非章内有明确的分隔标记。
2. **连续两章不给同一配角 POV。** 主角可连续，配角不行。
3. **POV 切换必须在章首第一段明确新视角角色。** 不可让读者猜了三段才发现换人了。
4. **全篇 POV 角色总数不超过 5 个。** 短篇/中篇 1-2 个，长篇 3-5 个。
5. **每个 POV 角色的首次出场，必须在前 30% 篇幅内完成。**

在大纲的开头部分确定全篇 POV 策略：使用哪种 POV 类型、POV 角色列表及首次出场章节、每个 POV 角色承担的叙事功能。

### 幕/卷划分

确定模型后，划分大的结构单元。每幕/卷需要明确：

1. **核心目标**：这一幕要完成什么叙事任务
2. **冲突升级**：相对上一幕，紧张程度如何递进
3. **主角弧线进展**：角色从什么状态走到什么状态
4. **揭示重点**：本幕需要揭示哪些世界观元素（必须对照揭示时间表）
5. **主题测试点**：本幕如何推进、挑战或呼应核心主题
6. **幕末转折**：以什么事件结束，制造下一幕的悬念

### 章节细化

幕级结构确定后，逐章填充。**不允许只做到幕级就停下。** 幕级大纲是半成品，章节级大纲才是可执行的蓝图。
...
每条伏笔独立存储为 `docs/dreampowers/tracking/` 目录下的一个 `.md` 文件，文件名使用 `thread-` 前缀（如 `thread-001-rusty-key.md`），通过符号链接分配到相关章节工作区。
...
**元数据字段说明：**
- `type`：明线 / 暗线 / 草蛇灰线
- `tier`：大纲规划的预期回收章节范围（如"第10-15章"、"第二幕末"）
...
`docs/dreampowers/chapters/chapter-NNN/` 是 AI 写作时的**唯一信息来源**。章节工作区完全自包含，包含：

1. **符号链接** — 指向 `set/concept/`、`set/character/`、`tracking/thread-*` 中的源文件
2. **`spec.md`** — 章节元数据
3. **`draft.md`** — 章节草稿（写作阶段产生）
4. **`review.md`** — 章节审查报告（审查阶段产生）
5. **`iron-rules.md`** — 指向 `tracking/iron-rules.md` 的符号链接
6. **`style.md`** — 指向 `tracking/style.md` 的符号链接
...
- 前序摘要链接：**不在大纲阶段创建**。由 `dp-chapter-draft` 在每章写作前动态创建 `summary-NNN.md → ../../timeline/summary-NNN.md`，取 `timeline/` 中已存在的最近 1-3 章摘要，不足 3 章按实际数量链接

### iron-rules.md

六条铁律的精简版，存储在 `docs/dreampowers/tracking/iron-rules.md`，通过符号链接出现在每个章节工作区中。AI 在写作时读取此文件，确保铁律贯穿整个写作链条。
...
`spec.md` 是章节工作区中唯一的非符号链接元数据文件，使用纯 Markdown 编写：
...
**概念预算**：
- `新概念上限`：本章允许首次引入的新概念数量。第一章通常为 3，后续章节通常为 2
- `深化概念上限`：本章允许深化的已有概念数量。通常不限
- `本章新引入`：列出本章计划首次引入的概念名称
- `本章深化`：列出本章计划深化展开的已有概念名称

**门控标准**：
- 清单格式。`dp-chapter-draft` 的 Pre-Draft Gate 逐项检查
- 可包含本章特有的检查项（如"本章使用倒叙手法，确认回溯锚点清晰"）

**概念依赖**：
- 描述概念之间的前置关系。概念 B 依赖概念 A 意味着读者需要先接触 A 才能理解 B
- **小说不是说明文**：依赖不意味着需要完整解释。概念可以只露出冰山一角就满足依赖条件

**读者评估要求**：
- 定义本章期望达到的读者体验标准
- 由 `dp-review-reader` 在审查时对照执行

**改进要求**：
- 如果本章有来自前序审查的改进反馈，记录在此
...
- `set/concept/` 和 `set/character/` 中的每个源文件，是否都至少在某个章节工作区中有符号链接？（未被引用的概念/角色要么是遗漏，要么应移除）
- 每章工作区中的符号链接数量是否合理？
...
│   └── review-*.md                  # 阶段性全书审查
└── chapters/
    └── chapter-NNN/                 # 章节工作区（本技能创建）
        ├── spec.md
        ├── iron-rules.md          → ../../tracking/iron-rules.md
        ├── style.md               → ../../tracking/style.md
        ├── summary-*.md           → ../../timeline/summary-*.md  (dp-chapter-draft 动态创建)
        ├── [concept].md           → ../../set/concept/[concept].md
        ├── [character].md         → ../../set/character/[character].md
        └── thread-*.md            → ../../tracking/thread-*.md
```

大纲文件存储至：`docs/dreampowers/outlines/outline-YYYY-MM-DD-<story-name>.md`
...
<HARD-GATE>
章节写作必须严格串行推进，适用于所有 dp-* 技能。写完第 N 章、确认无误后，才能开始第 N+1 章。禁止并行写多章。
</HARD-GATE>
...
- ❌ **不要只做幕级大纲就交付。** 没有章节细化的大纲无法指导 dp-chapter-draft。
- ❌ **不要跳过交叉验证。** "感觉差不多"不是验证。
...
| 关系 | 技能 | 说明 |
|------|------|------|
| 上游 | `dp-set-concept` | 接收拆分后的概念/角色源文件 + 故事级时间线（`timeline/timeline.md`） |
| 上游 | `dp-set-style` | 接收风格档案（`tracking/style.md`），创建章节工作区时添加 `style.md` 符号链接 |
