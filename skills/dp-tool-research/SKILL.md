...
本技能是**刚性技能**。前提文档必须经过用户确认才能推进到下一阶段。

通过自然对话和定向考据，把用户脑中一个模糊的灵感变成自洽、完整的故事蓝本。整个过程分为两个交替的阶段：**构思**（定方向、选冲突、塑角色）和**考据**（验证真实性、充实细节、确保设定可信）。两者不是线性流程，而是根据需要随时交替。

**硬性约束：** 在用户明确确认故事蓝本之前，禁止调用任何实现类技能。不得调用 `skill("dp-set-concept")`、`skill("dp-set-outline")` 或任何后续技能。不论故事看起来多简单，蓝本必须经过确认才能推进。
...
6. **确定故事名称** — 好名字是读者点开的第一理由，此时确定可以贯穿后续全部技能模板
7. **识别需要考据的元素** — 标记故事中涉及的真实世界元素
8. **执行定向考据** — 针对标记元素进行搜索验证
9. **提出 2-3 个故事方向** — 含考据成果、取舍分析和推荐
10. **撰写故事蓝本文档**（一句话故事概要 + 故事名 + 主角 + 核心冲突 + 主题 + 基调 + 考据要点）
11. **蓝本审查** — 派遣审查子 agent 检查蓝本的可行性和自洽性
12. **用户确认蓝本** — 等待明确批准
13. **过渡到下一技能** — 调用 `skill("dp-set-style")`
...
故事名称在蓝本阶段确定，之后贯穿所有下游技能模板（`dp-set-concept`、`dp-set-outline`、`dp-chapter-draft` 等均引用 `[故事名]`）。

**命名策略（按用户偏好选择）：**

1. **用户已有名字** — 直接采用，检查是否与类型/基调匹配
2. **AI 提议** — 基于已确定的类型、冲突、基调，提出 3-5 个候选名
3. **暂定名** — 用户暂时没想法时用工作代号（如"项目A"），在蓝本中标注为暂定，后续可改

**好名字的特征：**

- 与故事类型和基调一致（仙侠小说叫《摩天大楼》会让读者困惑）
- 引发好奇或画面感（《三体》《盗墓笔记》《鬼吹灯》）
- 目标平台的读者能一眼识别类型
- 长度适中（2-6 字为佳，超过 8 字的书名记忆成本高）

**名字可以改。** 蓝本阶段定的是工作名称。如果写到后期觉得名字不合适，在 `dp-review-final-report` 终检的"标题定稿"环节最终确认。
...
- **一句话故事概要是否清晰？** 不懂这个故事的人读完能不能知道这是写什么的？
...
当人工作者阅读了已完成的若干章节后，可能希望对后续章节的方向进行微调。本技能提供**作者调优**模式，将调优意见写入目标章节文件夹的 `tuning.md` 文件。
...
3. **收集调优意见** — 通过对话了解用户想调整什么：
   ...
   - 基调需要转变？
   - 其他任何方向性调整
4. **撰写 tuning.md** — 将调优意见写入目标章节文件夹。对每个目标章节，在 `docs/dreampowers/chapters/chapter-NNN/` 目录下创建 `tuning.md` 文件。如果章节文件夹尚未建立（大纲阶段未执行），先提示用户需要先完成大纲
...
## 优先级
铁律 > tuning.md > adult.md > style.md > 大纲默认设定 > spec.md 中的一般指导。
```
...
- 调优指令的优先级：铁律 > tuning.md > adult.md > style.md > 大纲默认设定 > spec.md 中的一般指导。
