...
本技能是**弹性技能**。核心原则不变，具体细节可因故事类型、角色数量、文体风格而调整。

章节结构：核心定位 → 前置条件 → 角色风格档案模板 → 角色风格创建流程(graphviz) → 对话写作规则（结构规则/对话标签规则/潜台词写作指引）→ 角色风格一致性检查 → 潜台词深潜（核心定位/适用时机/四种来源/构建技法/密度控制/审查方法/构建流程）→ 交互表 → 反模式 → 注意事项 → 终止状态
...
| 规则 | 阈值 | 说明 |
|------|------|------|
| 连续对话上限 | ≤5 个来回 | 超过 5 个来回必须插入叙事/动作/心理描写（详见 `dp-chapter-direct`） |
